...
-- ──────────────────────
-- ✓ update_updated_at() + set_updated_at() → single set_updated_at() function
-- ✓ pgcrypto extension declared once
-- ✓ social_links + contact_social_links → unified social_links (sections[])
-- ✓ contact_feature_flags (JSONB) → folded into feature_flags (boolean rows)
-- ✓ footer_stats + stats_items + careers_stats → page_stats (section column)
-- ✓ careers_hero + contact_hero → page_heroes (page column)
-- ✓ careers_section_headings (9-col row) → site_config rows (category='careers')
-- ✓ contact_page_copy (15-col row)       → site_config rows (category='contact')
-- ✓ contact_office_hours table was never created — fixed
-- ✓ ENUM types guarded (DO $$ … EXCEPTION duplicate_object)
-- ✓ All FK relationships explicit
-- ✓ Consistent naming: snake_case, singular table names where it reads better
--
-- EXECUTION ORDER (respect FK deps):
--   00 → 01 → 02 → 03 → 04 → 05 → 06 → 07 → 08 → 09
-- ═══════════════════════════════════════════════════════════════════════════


-- ══════════════════════════════════════════════════════════════════════════
-- 00. FOUNDATION  — extensions, shared function, shared enums
-- ══════════════════════════════════════════════════════════════════════════

CREATE EXTENSION IF NOT EXISTS "pgcrypto";      -- gen_random_uuid(); declared ONCE


-- Single canonical trigger function (was duplicated as update_updated_at
-- in migrations 003/005 and set_updated_at in gallery/careers/contact).
...
-- Helper: idempotent trigger creation
-- Usage: SELECT create_trigger_if_missing('trg_foo_upd','foo','set_updated_at');
...
-- ── Shared enum types ──────────────────────────────────────────────────────
-- Guard each with DO block so re-running the file is safe.
...
--     Three tables that every section reads from; no page owns them.
-- ══════════════════════════════════════════════════════════════════════════

-- ── 01a. site_config ──────────────────────────────────────────────────────
-- Key-value store for all scalar CMS strings and URLs.
-- category groups keys for admin UI panels; is_public controls API exposure.
--
-- Replaces separate one-row tables:
--   • careers_section_headings  → category = 'careers'
--   • contact_page_copy         → category = 'contact'
...
COMMENT ON TABLE site_config IS
  'Global scalar CMS settings. Use category to group in admin panels. '
  'Replaces careers_section_headings and contact_page_copy one-row tables.';
...
INSERT INTO site_config (key, value, value_type, label, category) VALUES
  ...
  -- Careers section headings  (replaces careers_section_headings table)
  ...
  -- Contact page copy  (replaces contact_page_copy table)
...
-- ── 01b. feature_flags ────────────────────────────────────────────────────
-- Single boolean flag table for ALL sections.
-- Replaces contact_feature_flags (JSONB single-row table) — now just rows here.
-- Convention: key = '<section>.<feature_name>'
...
COMMENT ON TABLE feature_flags IS
  'Global boolean feature flags. All sections use this table. '
  'Key convention: section.feature_name (e.g. header.ticker, contact.map).';
...
INSERT INTO feature_flags (key, is_enabled, description) VALUES
  ...
  -- Contact  (was a separate JSONB table; now normalised here)
...
-- ── 01c. social_links ─────────────────────────────────────────────────────
-- Unified table replacing:
--   • social_links         (header migration)
--   • contact_social_links (contact migration)
-- sections[] controls where each link is shown (header/footer/contact).
-- Same platform can appear once; sections array drives display placement.

CREATE TABLE IF NOT EXISTS social_links (
  id         SERIAL       PRIMARY KEY,
  platform   VARCHAR(60)  NOT NULL UNIQUE,
  url        TEXT         NOT NULL,
  icon_name  VARCHAR(60),                          -- lucide icon name
  sections   TEXT[]       NOT NULL DEFAULT '{}',  -- e.g. ARRAY['header','footer','contact']
...
COMMENT ON TABLE social_links IS
  'Unified social link table. sections[] controls which page areas render a given link. '
  'Replaces both social_links (header) and contact_social_links (contact).';

CREATE INDEX idx_social_links_sections ON social_links USING GIN (sections);

SELECT create_updated_at_trigger('trg_social_links_upd', 'social_links');

-- Merge: header had YouTube, contact had Instagram → union kept, sections set accordingly.
...
CREATE INDEX idx_nav_items_parent ON nav_items (parent_id, sort_order) WHERE is_active = TRUE;
...
-- ── 02b. announcements ────────────────────────────────────────────────────
-- Renamed from ticker_announcements; same shape, more generic name.
...
CREATE TABLE IF NOT EXISTS announcements (
  ...
  text       TEXT         NOT NULL,
  link_url   TEXT,
  section    VARCHAR(40)  NOT NULL DEFAULT 'header',   -- 'header', 'footer', etc.
  sort_order SMALLINT     NOT NULL DEFAULT 0,
  is_active  BOOLEAN      NOT NULL DEFAULT TRUE,
...
CREATE INDEX idx_announcements_active
  ON announcements (section, sort_order)
...
--     These tables serve multiple pages / sections.
-- ══════════════════════════════════════════════════════════════════════════

-- ── 03a. page_heroes ──────────────────────────────────────────────────────
-- Replaces careers_hero + contact_hero (identical structure).
-- One row per page; page column is the natural PK surrogate.

CREATE TABLE IF NOT EXISTS page_heroes (
  id               UUID         PRIMARY KEY DEFAULT gen_random_uuid(),
  page             VARCHAR(40)  NOT NULL UNIQUE,   -- 'careers', 'contact', 'gallery'
...
COMMENT ON TABLE page_heroes IS
  'Single-banner hero for interior pages (careers, contact, gallery, …). '
  'Replaces separate careers_hero and contact_hero tables.';
...
INSERT INTO page_heroes (page, headline, subheadline, description, cta_label, cta_href) VALUES
  (
    'careers',
    'Shape the Future of Pure Dairy',
    'Join a team that believes great milk starts with great people.',
    'At Kashee Milk, we are on a mission to deliver the purest, most nourishing dairy to every home in India. Behind every bottle is a team of passionate individuals — farmers, technologists, marketers, and dreamers — who show up every day to do meaningful work. If that sounds like you, we want to hear from you.',
    'Explore Open Roles',
    '#openings'
  ),
  (
    ...
    'Whether you have a question about our products, a partnership idea, or just want to say hello — we are here.',
    NULL, NULL, NULL
  )
...
-- ── 03b. page_stats ───────────────────────────────────────────────────────
-- Replaces three separate tables:
--   • footer_stats  (section='footer')  — display strings only
--   • stats_items   (section='home')    — integers + suffix for animated counter
--   • careers_stats (section='careers') — display strings only
--
-- numeric_value is nullable: populated only where an animated JS counter
-- is needed (section='home'). display_value always rendered as-is.

CREATE TABLE IF NOT EXISTS page_stats (
  id             SERIAL       PRIMARY KEY,
  section        VARCHAR(40)  NOT NULL,            -- 'home', 'footer', 'careers'
  stat_key       VARCHAR(60)  NOT NULL,
  display_value  VARCHAR(30)  NOT NULL,            -- "10K+", "45,000+", "200+"
  numeric_value  INTEGER,                          -- for JS counter; NULL if not used
  suffix         VARCHAR(10)  NOT NULL DEFAULT '', -- "+", "%", "cr"
  label          VARCHAR(120) NOT NULL,
...
COMMENT ON TABLE page_stats IS
  'Consolidated stats table. section column scopes each stat to its page area. '
  'Replaces footer_stats, stats_items, and careers_stats.';

SELECT create_updated_at_trigger('trg_page_stats_upd', 'page_stats');

-- Home page stats (animated counter — numeric_value populated)
...
-- Footer stats strip (display strings, no counter animation)
...
INSERT INTO hero_slides
  (image_url, image_alt, eyebrow, title, title_accent, description, cta_label, cta_href, tag_label, sort_order)
VALUES
...
CREATE TABLE IF NOT EXISTS about_paragraphs (
  id         SERIAL       PRIMARY KEY,
  content    TEXT         NOT NULL,  -- supports <strong> for bold
  sort_order SMALLINT     NOT NULL DEFAULT 0,
  is_active  BOOLEAN      NOT NULL DEFAULT TRUE,
  created_at TIMESTAMPTZ  NOT NULL DEFAULT NOW()
);
...
INSERT INTO about_news_items (title, image_url, href, source, sort_order) VALUES
  ('Kashee Milk Producer Organization to achieve turnover of Rs 200 cr by FY24',
   'https://www.kasheemilk.com/wp-content/uploads/2023/11/The-Economic-Times.jpg',
   '/kashee-milk-producer-organization-to-achieve-turnover-of-rs-200-cr-by-fy24-the-economic-times',
   'The Economic Times', 10),
  ('समृद्धि की अनूठी मिसाल पेश कर रहीं KMPO से जुड़ी महिलाएं',
   'https://www.kasheemilk.com/wp-content/uploads/2023/11/punjab-kesari.jpg',
   '#', 'Punjab Kesari', 20),
  ('इन महिलाओं ने दूध बेचकर कमाए लाखों रुपये',
   'https://www.kasheemilk.com/wp-content/uploads/2023/11/Agro-Haryana.jpg',
   '#', 'Agro Haryana', 30),
  ('समृद्धि की अनूठी मिसाल',
   'https://www.kasheemilk.com/wp-content/uploads/2023/11/The-Print-Hindi.jpg',
   '#', 'The Print Hindi', 40),
  ('KMPO से जुड़ी 2000 महिलाएं डेढ़ साल के भीतर बनी लाखों की मालकिन',
   'https://www.kasheemilk.com/wp-content/uploads/2023/11/ETV-Bharat.jpg',
   '#', 'ETV Bharat', 50)
ON CONFLICT DO NOTHING;
...
-- ── 05b. footer_services ──────────────────────────────────────────────────
-- Kept separate from nav_items: footer services column is independently
-- managed for display order / labelling without touching the main nav tree.
CREATE TABLE IF NOT EXISTS footer_services (
  id         SERIAL       PRIMARY KEY,
  label      VARCHAR(120) NOT NULL,
  href       VARCHAR(300) NOT NULL DEFAULT '#',
  nav_item_id INT         REFERENCES nav_items(id) ON DELETE SET NULL, -- optional link
  sort_order SMALLINT     NOT NULL DEFAULT 0,
  is_active  BOOLEAN      NOT NULL DEFAULT TRUE
);
...
CREATE INDEX idx_gallery_items_category ON gallery_items (category, is_active, sort_order, shot_date DESC);
CREATE INDEX idx_gallery_items_featured ON gallery_items (featured, is_active) WHERE featured = TRUE;
CREATE INDEX idx_gallery_items_tags     ON gallery_items USING GIN (tags);
CREATE INDEX idx_gallery_items_created  ON gallery_items (created_at DESC, is_active);
...
CREATE INDEX idx_job_openings_dept   ON job_openings (department_id, is_active, posted_at DESC);
CREATE INDEX idx_job_openings_feat   ON job_openings (is_featured, is_active) WHERE is_featured = TRUE;
CREATE INDEX idx_job_openings_tags   ON job_openings USING GIN (tags);

SELECT create_updated_at_trigger('trg_job_openings_upd', 'job_openings');

-- Sample openings (abbreviated for conciseness — full JDs unchanged from original)
WITH dept AS (SELECT id FROM departments WHERE slug = 'engineering' LIMIT 1)
INSERT INTO job_openings (title, slug, department_id, employment_type, experience_level, work_mode, location, salary_min, salary_max, salary_visible, summary, description, responsibilities, requirements, nice_to_have, tags, is_active, is_featured)
SELECT 'Senior Full-Stack Engineer','senior-fullstack-engineer', dept.id,'full_time','senior','hybrid','Varanasi, Uttar Pradesh',1800000,2800000,TRUE,
  ...
  ARRAY['nextjs','typescript','nodejs','postgresql','aws'],TRUE,TRUE FROM dept;

WITH dept AS (SELECT id FROM departments WHERE slug = 'engineering' LIMIT 1)
INSERT INTO job_openings (title, slug, department_id, employment_type, experience_level, work_mode, location, salary_visible, summary, description, responsibilities, requirements, nice_to_have, tags, is_active, is_featured)
SELECT 'DevOps Engineer','devops-engineer', dept.id,'full_time','mid','remote','Remote (India)',FALSE,
  ...
  ARRAY['devops','kubernetes','aws','terraform','cicd'],TRUE,FALSE FROM dept;

WITH dept AS (SELECT id FROM departments WHERE slug = 'product' LIMIT 1)
INSERT INTO job_openings (title, slug, department_id, employment_type, experience_level, work_mode, location, salary_min, salary_max, salary_visible, summary, description, responsibilities, requirements, nice_to_have, tags, is_active, is_featured)
SELECT 'Product Manager — Consumer App','product-manager-consumer-app', dept.id,'full_time','senior','hybrid','Varanasi, Uttar Pradesh',1600000,2400000,TRUE,
  ...
  ARRAY['product','consumer-app','growth','mobile'],TRUE,TRUE FROM dept;

WITH dept AS (SELECT id FROM departments WHERE slug = 'sales-marketing' LIMIT 1)
INSERT INTO job_openings (title, slug, department_id, employment_type, experience_level, work_mode, location, salary_visible, summary, description, responsibilities, requirements, nice_to_have, tags, is_active, is_featured)
SELECT 'Regional Sales Manager — North India','regional-sales-manager-north', dept.id,'full_time','lead','onsite','Lucknow / Varanasi, Uttar Pradesh',FALSE,
  ...
  ARRAY['sales','fmcg','north-india','leadership'],TRUE,TRUE FROM dept;

WITH dept AS (SELECT id FROM departments WHERE slug = 'quality' LIMIT 1)
INSERT INTO job_openings (title, slug, department_id, employment_type, experience_level, work_mode, location, salary_visible, summary, description, responsibilities, requirements, nice_to_have, tags, is_active, is_featured)
SELECT 'Quality Assurance Lead — Processing Plant','qa-lead-processing-plant', dept.id,'full_time','mid','onsite','Varanasi Processing Facility, UP',FALSE,
  ...
  ARRAY['quality','food-safety','dairy','haccp'],TRUE,FALSE FROM dept;

WITH dept AS (SELECT id FROM departments WHERE slug = 'hr' LIMIT 1)
INSERT INTO job_openings (title, slug, department_id, employment_type, experience_level, work_mode, location, salary_visible, summary, description, responsibilities, requirements, nice_to_have, tags, is_active, is_featured)
SELECT 'HR & Talent Intern','hr-talent-intern', dept.id,'internship','entry','hybrid','Varanasi, Uttar Pradesh',FALSE,
  ...
  ARRAY['hr','internship','talent','recruiting'],TRUE,FALSE FROM dept;
...
CREATE TABLE IF NOT EXISTS job_applications (
  ...
  -- Professional
  current_job_role       VARCHAR(200),    -- renamed: current_role is a PostgreSQL reserved word
...
CREATE INDEX idx_applications_job_status ON job_applications (job_id, status, submitted_at DESC);
CREATE INDEX idx_applications_email      ON job_applications (email);
CREATE INDEX idx_applications_status     ON job_applications (status, submitted_at DESC);
...
INSERT INTO contact_offices (label, address_line1, address_line2, city, state, pincode, phone, email, map_embed_url, map_link_url, latitude, longitude, is_primary, sort_order) VALUES
  (
    ...
    'Varanasi Industrial Estate, Rohaniya',
    'Varanasi', 'Uttar Pradesh', '221109',
    '+91 98765 43210', 'info@kasheemilk.com',
    'https://www.google.com/maps/embed?pb=!1m18!1m12!1m3!1d3607.5!2d82.9739!3d25.3176!2m3!1f0!2f0!3f0!3m2!1i1024!2i768!4f13.1',
    'https://maps.google.com/?q=Varanasi+Industrial+Estate+Rohaniya+Varanasi+UP',
    25.3176, 82.9739, TRUE, 1
  )
ON CONFLICT DO NOTHING;


-- ── 08b. contact_office_hours ─────────────────────────────────────────────
-- This table was MISSING from the original migrations (INSERT existed, CREATE did not).

CREATE TABLE IF NOT EXISTS contact_office_hours (
  id          SERIAL       PRIMARY KEY,
  office_id   UUID         REFERENCES contact_offices(id) ON DELETE CASCADE,
  days_label  VARCHAR(60)  NOT NULL,    -- "Monday – Friday"
  hours_label VARCHAR(60)  NOT NULL,    -- "9:00 AM – 6:00 PM"
  is_closed   BOOLEAN      NOT NULL DEFAULT FALSE,
  sort_order  SMALLINT     NOT NULL DEFAULT 0
);

COMMENT ON TABLE contact_office_hours IS
  'Operating hours per office. office_id NULL = applies to all offices (global default).';

-- Global default hours (office_id = NULL → applies to all offices)
...
CREATE INDEX idx_contact_submissions_status ON contact_submissions (status, submitted_at DESC);
CREATE INDEX idx_contact_submissions_email  ON contact_submissions (email, submitted_at DESC);
...
-- ══════════════════════════════════════════════════════════════════════════
-- 09. VIEWS
--     Lean, purposeful aggregations. No business logic in views.
-- ══════════════════════════════════════════════════════════════════════════
...
COMMENT ON VIEW vw_header_nav IS 'Nav tree for header dropdown — top-level items with children JSON array.';
...
-- ── Unread contact submissions (admin) ────────────────────────────────────
...
-- ── Gallery active items with category label ──────────────────────────────
...
-- ══════════════════════════════════════════════════════════════════════════
-- QUICK SANITY CHECKS (uncomment to run after migration)
-- ══════════════════════════════════════════════════════════════════════════

-- SELECT COUNT(*) AS total_config_keys     FROM site_config;
-- SELECT COUNT(*) AS total_feature_flags   FROM feature_flags;
-- SELECT platform, sections                FROM social_links ORDER BY sort_order;
-- SELECT section, COUNT(*) AS n            FROM page_stats GROUP BY section ORDER BY section;
-- SELECT page, headline                    FROM page_heroes ORDER BY page;
-- SELECT * FROM vw_gallery_category_counts;
-- SELECT title, department_name            FROM vw_active_jobs;
-- SELECT * FROM vw_footer_data;
-- SELECT label, days_label, hours_label    FROM contact_office_hours coh
--   LEFT JOIN contact_offices co ON co.id = coh.office_id ORDER BY sort_order;
